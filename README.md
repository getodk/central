...
If you want to work on Central codebase and don't want to setup dependent services like Postgresql, Enketo, etc manually then you can run `make for-central-dev`, which will start those services as Docker containers. Similarly you can run `make for-enketo-dev` if you wish to work on Enketo Express and need to setup Central locally. If you are setting up either development environment for the first time, please run `make upgrade-successful` to skip the Postgresql upgrade.
